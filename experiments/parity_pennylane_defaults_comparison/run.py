#!/usr/bin/env python3
"""Strict-default PennyLane optimizer comparison on the parity circuit."""
...
from qml_models.variants.projected_model import ProjectedTrainableModel
...
def _timestamped_dir(results_root: str, run_name: str | None) -> str:
    ...
    suffix = run_name or "parity_pennylane_defaults"
...
def _build_no_bias_krotov_model(seed: int, n_layers: int):
    base = ParityRotClassifierModel(n_layers=n_layers)
    full_init = np.asarray(base.init_params(seed=seed), dtype=float)
    quantum_indices = np.arange(base.n_quantum_params, dtype=int)
    projected = ProjectedTrainableModel(
        base,
        full_reference_params=full_init,
        trainable_indices=quantum_indices,
        label="ParityRotNoBias",
    )
    return base, projected, full_init
...
class PennyLaneParityNoBiasClassifier:
    ...
    def init_params(self, seed: int):
        ...
        return pnp.array(native[:-1].reshape(self.weights_shape), requires_grad=True), native[:-1]

    def predict_scores(self, weights, X):
        return pnp.array([self.circuit(x_bits, weights) for x_bits in X])

    def loss(self, weights, X, y):
        scores = self.predict_scores(weights, X)
...
def _run_krotov(seed: int, args, X_train, y_train, X_test, y_test):
    base_model, model, full_init = _build_no_bias_krotov_model(seed, args.n_layers)
    init_params = full_init[: base_model.n_quantum_params].copy()
...
def _run_pennylane_optimizer(optimizer_name: str, seed: int, args, X_train, y_train, X_test, y_test):
    model = PennyLaneParityNoBiasClassifier(n_layers=args.n_layers)
    weights, native_init = model.init_params(seed=seed)
    ...
    if optimizer_name == "pennylane_adam":
        ...
        raise ValueError(f"Unknown strict-default optimizer: {optimizer_name}")

    cost_fn = lambda w: model.loss(w, X_train_bits, y_train_arr)
    ...
    for step in range(1, args.max_iterations + 1):
        ...
        train_scores = model.predict_scores(weights, X_train_bits)
        test_scores = model.predict_scores(weights, X_test_bits)
    ...
    return {
        ...
        "final_params": np.asarray(weights, dtype=float).reshape(-1).tolist(),
        "trace": trace,
    }
...
def main():
    ...
    for seed in args.seeds:
        ...
        print(f"\n{'=' * 88}")
        print(f"Strict-default parity comparison | seed={seed} | repeats={args.repeats} | layers={args.n_layers}")
        print(f"{'=' * 88}")
    ...
    title = f"4-bit parity no-bias, {args.n_layers} layers, repeats={args.repeats}"
    _plot_metric(
        all_results,
        "step",
        "loss",
    ...
    print(f"\nSaved strict-default parity comparison to {results_dir}/")
