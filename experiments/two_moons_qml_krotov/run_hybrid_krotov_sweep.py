...
def load_iris_binary(test_fraction=0.2, seed=42):
    ...
    return (
        np.pi * (X_train - x_min) / span,
        np.pi * (X_test - x_min) / span,
        y_train,
        y_test,
    )
...
# ── Sweep runner ──────────────────────────────────────────────────────────
def run_sweep(model_key, results_dir):
    spec = MODEL_SPECS[model_key]
    ...
    out_path = os.path.join(results_dir, f"sweep_{model_key}.json")
...
# ── Analysis ──────────────────────────────────────────────────────────────
def analyze_sweep(model_key, results, results_dir):
    spec = MODEL_SPECS[model_key]
    threshold = spec["loss_threshold"]
    ...
    out_path = os.path.join(results_dir, f"analysis_{model_key}.json")
    with open(out_path, "w") as f:
        json.dump(summary, f, indent=2)

    print(f"\n{'=' * 72}")
    print(f"  {spec['label']} — Top 10 configurations (by mean final loss)")
    print(f"{'=' * 72}")
...
# ── Plotting ──────────────────────────────────────────────────────────────
def generate_plots(model_key, results, summary, results_dir):
    import matplotlib
    matplotlib.use("Agg")
    ...
    plt.tight_layout()
    fig.savefig(os.path.join(results_dir, f"sweep_{model_key}_heatmap.pdf"), bbox_inches="tight")
    fig.savefig(os.path.join(results_dir, f"sweep_{model_key}_heatmap.png"), dpi=200, bbox_inches="tight")
    plt.close(fig)
    ...
    plt.tight_layout()
    fig.savefig(os.path.join(results_dir, f"sweep_{model_key}_bar.pdf"), bbox_inches="tight")
    fig.savefig(os.path.join(results_dir, f"sweep_{model_key}_bar.png"), dpi=200, bbox_inches="tight")
    plt.close(fig)

    print(f"  Plots saved: sweep_{model_key}_heatmap.pdf, sweep_{model_key}_bar.pdf")


# ── Report ────────────────────────────────────────────────────────────────
def write_report(summaries, results_dir):
    lines = [
    ...
    report_path = os.path.join(results_dir, "sweep_report.md")
...
def main():
    ...
    for model_key in args.models:
        results = run_sweep(model_key, args.results_dir)
        summary = analyze_sweep(model_key, results, args.results_dir)
        generate_plots(model_key, results, summary, args.results_dir)
