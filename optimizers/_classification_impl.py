...
- ``adam``: Adam with full-batch exact gate-state backpropagation gradients
- ``lbfgs``: L-BFGS-B with exact gate-state backpropagation gradients
...
from qml_models.losses import EPS
from ._krotov_scaling import build_scaling_strategy, get_gate_metadata, SCALING_APPLY_PHASES
...
def _select_training_batch(
    ...
    """Return the active training subset for an optimizer iteration.

    When ``batch_size`` and ``switch_iteration`` are enabled, the first
    ``switch_iteration`` updates use deterministic mini-batches sampled from a
    fresh per-iteration permutation. Later iterations fall back to the full
    training set and keep the legacy phase name.
    """
    if batch_size is None:
        return X_train, y_train, phase_name

    batch_size = int(batch_size)
    if batch_size <= 0:
        raise ValueError("Mini-batch size must be positive.")

    if switch_iteration <= 0 or iteration > switch_iteration or batch_size >= len(X_train):
        return X_train, y_train, phase_name

    rng = np.random.RandomState(iteration)
    batch_idx = rng.permutation(len(X_train))[:batch_size]
    return X_train[batch_idx], y_train[batch_idx], f"{phase_name}_minibatch"
...
def _krotov_single_sample_update(model, params, x, y, step_size,
                                   scaling_strategy=None, gate_meta_lookup=None):
    """Original sequential single-sample Krotov update with optional scaling.
    ...
    When *scaling_strategy* is provided, each gate-local gradient is multiplied
    by the strategy's per-parameter factor before the parameter update.
    ``sample_contribution`` always stores the raw (unscaled) gradient signal.
    """
    gates, fwd_states = model.get_gate_sequence_and_states(params, x)
    chi_states = _build_costates(
        gates, _krotov_terminal_costate(model, fwd_states[-1], y, params=params, x=x)
    ...
    for k, (gate_mat, pidx) in enumerate(gates):
        if pidx is not None:
            ...
            if scaling_strategy is not None:
                meta_entry = gate_meta_lookup.get(pidx) if gate_meta_lookup else None
                meta_list = [meta_entry] if meta_entry is not None else None
                factor = scaling_strategy.compute_scaling_factors(
                    np.array([gradient]), np.array([pidx]), model, meta_list
                )
                effective_gradient = gradient * factor[0]

            new_params[pidx] -= step_size * effective_gradient
...
def _run_krotov_online_epoch(model, params, X_train, y_train, step_size, iteration_seed,
    ...
    for idx in perm:
        params, contribution, stats = _krotov_single_sample_update(
            ...
            gate_meta_lookup=gate_meta_lookup,
        )
...
def _run_krotov_batch_epoch(model, params, X_train, y_train, step_size,
                             scaling_strategy=None, gate_indices=None, gate_metadata=None):
    """Apply one full-batch Krotov-inspired update with optional scaling."""
    mean_contribution, contribution_variance, counter_delta = _krotov_contribution_batch(
        model, params, X_train, y_train
    )
    if scaling_strategy is not None and gate_indices is not None and len(gate_indices) > 0:
        gate_update = mean_contribution[gate_indices]
        factors = scaling_strategy.compute_scaling_factors(
            gate_update, gate_indices, model, gate_metadata
        )
        mean_contribution[gate_indices] *= factors
    classical_grad = np.zeros_like(params)
    nongate_indices = (
        np.asarray(model.nongate_parameter_indices(), dtype=int)
        if hasattr(model, "nongate_parameter_indices")
...
def train_adam(
    ...
    """Train using Adam with exact BCE gradients.

    By default every step uses the full training set. When ``batch_size`` and
    ``switch_iteration`` are provided, the first ``switch_iteration`` updates
    use mini-batch gradients and later iterations revert to the original
    full-batch Adam rule.
    """
    trace = _init_trace()
    counters = _init_counters()
    ...
    for it in range(1, max_iterations + 1):
        grad_X, grad_y, phase = _select_training_batch(
            X_train,
            y_train,
            it,
            "adam",
        ...
        grad, grad_stats = model.loss_gradient(params, grad_X, grad_y)
...
def train_krotov_hybrid(
    ...
    When *scaling_mode* is not ``"none"``, gate-supported updates are multiplied
    by per-parameter factors computed by the chosen scaling strategy.
    *scaling_apply_phase* controls whether scaling is active during the online
    phase, the batch phase, or both.
    """
    if scaling_apply_phase not in SCALING_APPLY_PHASES:
        raise ValueError(
            f"Invalid scaling_apply_phase '{scaling_apply_phase}'. "
            f"Valid: {sorted(SCALING_APPLY_PHASES)}."
        )
    strategy = build_scaling_strategy(scaling_mode, scaling_config)
    ...
    gate_metadata = get_gate_metadata(model, gate_indices)
    strategy.validate(gate_indices, model, gate_metadata)

    gate_meta_lookup = None
    if gate_metadata is not None:
        gate_meta_lookup = {
            int(idx): meta
            for idx, meta in zip(gate_indices, gate_metadata)
            if meta is not None
        }

    is_active = scaling_mode != "none"
    apply_online = is_active and scaling_apply_phase in ("online", "both")
    apply_batch = is_active and scaling_apply_phase in ("batch", "both")
    online_strategy = strategy if apply_online else None
    online_meta_lookup = gate_meta_lookup if apply_online else None
    batch_strategy = strategy if apply_batch else None
    batch_gate_indices = gate_indices if apply_batch else None
    batch_gate_metadata = gate_metadata if apply_batch else None
    ...
    for it in range(1, max_iterations + 1):
        if it <= switch_iteration:
            ...
            params, diagnostics, counter_delta = _run_krotov_online_epoch(
                model, params, X_train, y_train, current_step, it,
                scaling_strategy=online_strategy,
                gate_meta_lookup=online_meta_lookup,
            )
        else:
            ...
            params, diagnostics, counter_delta = _run_krotov_batch_epoch(
                model, params, X_train, y_train, current_step,
                scaling_strategy=batch_strategy,
                gate_indices=batch_gate_indices,
                gate_metadata=batch_gate_metadata,
            )
...
def train_qng(
    ...
    """Train using Quantum Natural Gradient with exact BCE gradients.

    The Fubini-Study metric tensor is recomputed at every iteration on the same
    data used for the loss gradient. By default this is the full training set.
    When ``batch_size`` and ``switch_iteration`` are provided, the first
    ``switch_iteration`` updates use mini-batches for both the gradient and the
    metric tensor before reverting to the original full-batch QNG rule.

    Non-gate (classical) parameters receive a standard gradient-descent update
    ...
    for it in range(1, max_iterations + 1):
        ...
        MT, mt_stats = _compute_metric_tensor(
            model, params, grad_X, gate_indices, approx=approx, lam=lam,
        )
        ...
        _append_trace(
            trace, counters, it, phase, time.time() - t0, tl, ta, tea,
...
def run_optimizer(name, model, params, X_train, y_train, X_test, y_test, config):
    ...
    if name == "krotov_hybrid":
        ...
        return train_krotov_hybrid(
            ...
            early_stopping_min_delta=getattr(config, "early_stopping_min_delta", 0.0),
            early_stopping_warmup=getattr(config, "early_stopping_warmup", 0),
            scaling_mode=getattr(config, "hybrid_scaling_mode", "none"),
            scaling_apply_phase=getattr(config, "hybrid_scaling_apply_phase", "both"),
            scaling_config=getattr(config, "hybrid_scaling_config", None),
        )
    ...
    if name == "adam":
        return train_adam(
            ...
            max_iterations=config.max_iterations,
            lr=config.adam_lr,
            batch_size=getattr(config, "adam_batch_size", None),
            switch_iteration=getattr(config, "adam_switch_iteration", 0),
        )
    ...
    if name == "lbfgs":
        return train_lbfgs(
            ...
            maxcor=getattr(config, "lbfgs_maxcor", 20),
    ...
    if name == "qng":
        return train_qng(
            ...
            batch_size=getattr(config, "qng_batch_size", None),
            switch_iteration=getattr(config, "qng_switch_iteration", 0),
        )
