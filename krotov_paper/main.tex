...
\title{Hybrid Krotov Optimization for Quantum Machine Learning}
...
\begin{itemize}
    ...
    \item We report empirical evidence that the hybrid schedule combines the strong early progress of online Krotov with the stability of batch Krotov. On the original four-qubit two-moons benchmark this yields the clearest mechanistic explanation of the method.
...
\end{equation}
where \(S(t)\) is a shape function and \(\lambda_a\) is a regularization parameter. The defining conceptual feature is that the update respects the dynamical structure of the problem rather than treating the objective as a black-box function of the control.
...
\end{equation}
where \(p_n(\bm{\theta})\) is the model prediction for sample \(n\), \(y_n\in\{0,1\}\), and \(\ell\) is binary cross-entropy.
...
\end{equation}
then the loss derivative propagates through the classical head before defining the terminal costate.
...
For one training sample \((x_n,y_n)\), we compute the forward states, terminal costate, and backward costates, then update parameters gate by gate using the sample-wise contributions \(\Delta_{n,k}\). This variant is aggressive and often effective in the early training regime, but it may induce substantial late-stage oscillations.
